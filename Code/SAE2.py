...
for i in range(len(Listeregion)) :
    ...
    prodtotaleannee(Listeregion[i])
    plt.sca(axes[1])  # CASE 1 (AU MILIEU)
    prodvertannee(Listeregion[i])
    plt.sca(axes[2])  # CASE 2 (EN BAS)
    pctvertannee(Listeregion[i])
    fig.tight_layout()
    nom_fichier = "Annee/bilan_annuel_{Listregion[i]}.png"
    fig.savefig(nom_fichier, dpi=300)
    plt.close(fig)

'''
for i in range(len(Listeregion)) :
    for k in range(len(ListeAnnee)) :
        fig, axes = plt.subplots(3, 1, figsize=(16, 10))
        fiurebase = plt.figure
        showbase = plt.show
        try:
            plt.figure = noop
            plt.show = noop
            plt.sca(axes[0])  # CASE 0 (EN HAUT)
            prodtotalemois(ListeAnnee[k],Listeregion[i])
            plt.sca(axes[1])  # CASE 1 (AU MILIEU)
            constotalemens(ListeAnnee[k],Listeregion[i])
            plt.sca(axes[2])  # CASE 2 (EN BAS)
